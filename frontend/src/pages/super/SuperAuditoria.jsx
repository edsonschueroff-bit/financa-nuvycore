...
export default function SuperAuditoria() {
  ...
  const carregarLogs = async () => {
    try {
      ...
      const [logsRes, statsRes] = await Promise.all([
        api.get(`/auditoria/global?${params.toString()}`),
        api.get("/auditoria/global/estatisticas"),
      ]);
      ...
      console.error("Erro ao carregar logs globais de auditoria:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    carregarLogs();
  }, [page, empresaId, modulo, acao, dataInicio, dataFim]);
  ...
  const handleExportExcel = () => {
    ...
    const rows = logs.map((l) => ({
      ...
      "Empresa Assinante": l.empresa_nome || "—",
    ...
    exportToExcel(rows, "Auditoria_Global_SaaS", "Auditoria SaaS");
  };
  ...
  const handleExportCsv = () => {
    ...
    exportToCsv(rows, "Auditoria_Global_SaaS");
  };
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
              <Shield className="text-emerald-600" size={24} /> Auditoria Global da Plataforma SaaS
            </h1>
            <p className="text-xs text-slate-500 font-medium">
              Visão consolidada de acessos, alterações financeiras e operações de todas as empresas assinantes.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <Button
              ...
              title="Exportar logs globais para Excel (.XLSX)"
            >
              Excel
            </Button>
            <Button
              ...
              title="Exportar logs globais para CSV"
            >
              CSV
            </Button>
            <Button
        ...
        {/* Barra de Filtros com Seletor de Empresa */}
        <Card className="p-4">
          <form onSubmit={handleSearchSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3">
            ...
            <div className="relative">
              <input
                type="text"
                placeholder="Buscar usuário, email, IP, detalhe..."
                value={search}
          ...
          {(empresaId || search || modulo || acao || dataInicio || dataFim) && (
        ...
        {/* Tabela Principal de Logs Globais */}
        <Card className="overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
            <div>
              <h3 className="font-bold text-slate-900 text-sm">Registro Global de Atividades SaaS</h3>
            ...
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-slate-600 font-bold uppercase tracking-wider text-[10px]">
                  ...
                  <th className="px-4 py-3">Empresa Assinante</th>
                ...
                {loading ? (
                  <tr>
                    <td colSpan="8" className="px-4 py-12 text-center text-slate-400">
                      Carregando trilha global de auditoria...
                    </td>
                  </tr>
                ) : logs.length === 0 ? (
                  <tr>
          ...
          {modalDetalhes && (
            <div className="space-y-4 text-xs">
              <div className="grid grid-cols-2 gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
                ...
                <div>
                  <span className="text-slate-400 block font-bold text-[10px] uppercase">Empresa / Tenant</span>
                  <span className="font-bold text-emerald-700">
                    {modalDetalhes.empresa_nome || "Plataforma"}
                  </span>
                </div>
                <div>
